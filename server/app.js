...
class App {
  ...
  match(players) {
    ...
    this.io.to(pXSocketID).emit("info", `${msg_game_1} ${msg_player_x}`);
    this.io.to(pOSocketID).emit("info", `${msg_game_1} ${msg_player_o}`);
    this.io.to(roomID).emit("progress", newGame.progress);
    this.io.to(roomID).emit("scoreboard", JSON.stringify(newGame.scoreboard));
  }
  ...
  handleReplay(socket, confirmed) {
    ...
    if (!confirmed) {
      ...
      game.reset();
      game.init();
      this.io.to(roomID).emit("scoreboard", JSON.stringify(game.scoreboard));
